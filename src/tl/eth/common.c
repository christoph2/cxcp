...
void XcpTl_PrintConnectionInformation(void) {
  ...
  //    Curl_inet_ntop(Xcp_Options.ipv6 ? AF_INET6 : AF_INET,
  //    &XcpTl_Connection.localAddress, &buf, 128); printf("%s\n", buf);
}
...
/*
** NOTE: The following code is taken from OpenBSD, because inet_ntop() is
*notorious
...
static char *inet_ntop4(const unsigned char *src, char *dst, size_t size) {
  ...
  tmp[0] = '\0';
  (void)sprintf(tmp, "%d.%d.%d.%d", ((int)(src[0])) & 0xff,
                ((int)(src[1])) & 0xff, ((int)(src[2])) & 0xff,
                ((int)(src[3])) & 0xff);
