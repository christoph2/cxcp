...
void Kv_Error(const char *msg) { printf("ERROR[Kvaser]:: %s\n", msg); }

void Kv_Info(const char *msg) { printf("INFO[Kvaser]:: %s\n", msg); }
...
static void Kv_Notification(int hnd, void *context, unsigned int notifyEvent) {
  ...
  case canNOTIFY_RX:
    ...
    if (dlc > 0) {
      ...
      } else if (XcpTl_MatchingAddress(XCP_ON_CAN_BROADCAST_IDENTIFIER, id,
                                       flag)) {
...
void XcpTl_Init(void) {
  ...
  hnd = canOpenChannel(
      0, canOPEN_ACCEPT_VIRTUAL |
             canOPEN_NO_INIT_ACCESS /* | canOPEN_ACCEPT_LARGE_DLC*/);
  if (hnd < 0) {
  ...
  stat = kvSetNotifyCallback(hnd, (kvCallback_t)&Kv_Notification, NULL,
                             canNOTIFY_RX | canNOTIFY_TX | canNOTIFY_ERROR |
                                 canNOTIFY_STATUS | canNOTIFY_BUSONOFF);
  Kv_Check("kvSetNotifyCallback", stat);

  stat =
      canSetBusParams(hnd, XCP_ON_CAN_FREQ, XCP_ON_CAN_TSEG1, XCP_ON_CAN_TSEG2,
                      XCP_ON_CAN_SJW, XCP_ON_CAN_NOSAMP, 0);
...
static void XcpTl_SetCANFilter(void) {
  ...
  if ((XCP_ON_CAN_IS_EXTENDED_IDENTIFIER(XCP_ON_CAN_INBOUND_IDENTIFIER)) ||
      (XCP_ON_CAN_IS_EXTENDED_IDENTIFIER(XCP_ON_CAN_BROADCAST_IDENTIFIER))) {
    ext = 1;
    mask ^= 0x1fffffff;
  } else {
    ext = 0;
...
void XcpTl_PrintConnectionInformation(void) {
  ...
  stat = canGetChannelData(XcpTl_Connection.handle,
                           canCHANNELDATA_CHAN_NO_ON_CARD, &num, sizeof(num));
  Kv_Check("canGetChannelData", stat);
  stat = canGetChannelData(XcpTl_Connection.handle,
                           canCHANNELDATA_DEVDESCR_ASCII, &name, sizeof(name));
  Kv_Check("canGetChannelData", stat);
  printf("\nXCPonCAN -- %s (channel %d), listening on 0x%X [%s]\n", name, num,
         XCP_ON_CAN_STRIP_IDENTIFIER(XCP_ON_CAN_INBOUND_IDENTIFIER),
         ext ? "EXT" : "STD");
}
