...
XCP_STATIC bool
XcpDaq_AllocValidateTransition(XcpDaq_AllocTransitionype transition);
...
XCP_STATIC const uint8_t XcpDaq_AllocTransitionTable[5][4] = {
    ...
    /* ALLOC_IDLE*/ {UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_ERR),
                     UINT8(DAQ_ALLOC_ERR), UINT8(DAQ_ALLOC_ERR)},
    /* AFTER_FREE_DAQ  */
    {UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_ERR),
     UINT8(DAQ_ALLOC_ERR)},
    /* AFTER_ALLOC_DAQ */
    {UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_OK),
     UINT8(DAQ_ALLOC_ERR)},
    /* AFTER_ALLOC_ODT */
    {UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_ERR), UINT8(DAQ_ALLOC_OK),
     UINT8(DAQ_ALLOC_OK)},
    /* AFTER_ALLOC_ODT_ENTRY */
    {UINT8(DAQ_ALLOC_OK), UINT8(DAQ_ALLOC_ERR), UINT8(DAQ_ALLOC_ERR),
     UINT8(DAQ_ALLOC_OK)},
};
#endif /* XCP_DAQ_ENABLE_DYNAMIC_LISTS */
...
#if XCP_DAQ_ENABLE_MULTIPLE_DAQ_LISTS_PER_EVENT == XCP_OFF
  XcpUtl_MemSet(XcpDaq_ListForEvent, UINT8(0),
                UINT32(sizeof(XcpDaq_ListForEvent[0]) *
                       UINT8(XCP_DAQ_MAX_EVENT_CHANNEL)));
#endif /* XCP_DAQ_ENABLE_MULTIPLE_DAQ_LISTS_PER_EVENT */

  if (XcpDaq_AllocValidateTransition(XCP_CALL_FREE_DAQ)) {
    XcpUtl_MemSet(XcpDaq_Entities, UINT8(0),
                  UINT32(sizeof(XcpDaq_EntityType) *
                         (XCP_DAQ_ENTITY_TYPE)XCP_DAQ_MAX_DYNAMIC_ENTITIES));
...
Xcp_ReturnType XcpDaq_Alloc(XcpDaq_ListIntegerType daqCount) {
  ...
  if (!XcpDaq_AllocValidateTransition(XCP_CALL_ALLOC_DAQ)) {
    ...
    if ((XcpDaq_EntityCount + daqCount) <=
        (XCP_DAQ_ENTITY_TYPE)XCP_DAQ_MAX_DYNAMIC_ENTITIES) {
      XcpDaq_AllocState = XCP_AFTER_ALLOC_DAQ;
      for (idx = XcpDaq_EntityCount; idx < (XcpDaq_EntityCount + daqCount);
           ++idx) {
...
Xcp_ReturnType XcpDaq_AllocOdt(XcpDaq_ListIntegerType daqListNumber,
  ...
  if (!XcpDaq_AllocValidateTransition(XCP_CALL_ALLOC_ODT)) {
    ...
    if ((XcpDaq_EntityCount + odtCount) <=
        (XCP_DAQ_ENTITY_TYPE)XCP_DAQ_MAX_DYNAMIC_ENTITIES) {
      XcpDaq_AllocState = XCP_AFTER_ALLOC_ODT;
      for (idx = XcpDaq_EntityCount; idx < (XcpDaq_EntityCount + odtCount);
           ++idx) {
...
Xcp_ReturnType
XcpDaq_AllocOdtEntry(XcpDaq_ListIntegerType daqListNumber,
  ...
  if (!XcpDaq_AllocValidateTransition(XCP_CALL_ALLOC_ODT_ENTRY)) {
    ...
    if ((XcpDaq_EntityCount + odtEntriesCount) <=
        (XCP_DAQ_ENTITY_TYPE)XCP_DAQ_MAX_DYNAMIC_ENTITIES) {
      XcpDaq_AllocState = XCP_AFTER_ALLOC_ODT_ENTRY;
      for (idx = XcpDaq_EntityCount;
...
XcpDaq_AllocValidateTransition(XcpDaq_AllocTransitionype transition) {
  ...
  if (XcpDaq_AllocTransitionTable[XcpDaq_AllocState][transition] ==
      UINT8(DAQ_ALLOC_OK)) {
...
void XcpDaq_Init(void) {
  ...
  for (idx = (XcpDaq_ListIntegerType)0; idx < XcpDaq_PredefinedListCount;
       ++idx) {
...
#elif (XCP_DAQ_ENABLE_DYNAMIC_LISTS == XCP_OFF) &&                             \
    (XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON)
  return (XcpDaq_ODTEntryType *)&XcpDaq_PredefinedOdtEntries[idx];
  /* Predefined DAQs only */
#elif (XCP_DAQ_ENABLE_DYNAMIC_LISTS == XCP_ON) &&                              \
    (XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON)
...
XcpDaq_GetListState(XcpDaq_ListIntegerType daqListNumber) {
#if XCP_DAQ_ENABLE_DYNAMIC_LISTS == XCP_ON
...
bool XcpDaq_ValidateConfiguration(void) {
    ...
    (XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_OFF)
  /* Dynamic DAQs only */
  return (bool)((XcpDaq_EntityCount > (XCP_DAQ_ENTITY_TYPE)0) &&
                (XcpDaq_ListCount > (XCP_DAQ_ENTITY_TYPE)0) &&
                (XcpDaq_OdtCount > (XCP_DAQ_ENTITY_TYPE)0));
#elif (XCP_DAQ_ENABLE_DYNAMIC_LISTS == XCP_OFF) &&                             \
    (XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON)
...
void XcpDaq_TriggerEvent(uint8_t eventChannelNumber) {
  ...
  for (odtIdx = (XcpDaq_ODTIntegerType)0; odtIdx < listConf->numOdts;
       ++odtIdx) {
    offset = UINT16(0);
    ...
    for (odtEntryIdx = (XcpDaq_ODTEntryIntegerType)0;
         odtEntryIdx < odt->numOdtEntries; ++odtEntryIdx) {
      ...
      XcpDaq_CopyMemory(&data[offset], (void *)entry->mta.address,
                        entry->length);
      offset += entry->length;
    }
    pid++;
...
void XcpDaq_GetProperties(uint8_t *properties) {
  ...
  *properties |= XCP_DAQ_PROP_PRESCALER_SUPPORTED;
    ...
    (XCP_DAQ_CONFIG_TYPE == XCP_DAQ_CONFIG_TYPE_STATIC)
...
XCP_STATIC void XcpDaq_StartStopLists(XcpDaq_ListTransitionType transition) {
  ...
  for (idx = (XcpDaq_ListIntegerType)0; idx < XcpDaq_GetListCount(); ++idx) {
    ...
    if ((entry->mode & XCP_DAQ_LIST_MODE_SELECTED) ==
        XCP_DAQ_LIST_MODE_SELECTED) {
...
XCP_STATIC bool XcpDaq_QueueFull(void) {
  return ((XcpDaq_Queue.head + UINT8(1)) % UINT8(XCP_DAQ_QUEUE_SIZE + 1)) ==
         XcpDaq_Queue.tail;
}
...
bool XcpDaq_QueueEnqueue(uint16_t len, uint8_t const *data) {
  ...
  XcpDaq_Queue.head =
      (XcpDaq_Queue.head + UINT8(1)) % UINT8(XCP_DAQ_QUEUE_SIZE + 1);
  return (bool)XCP_TRUE;
}
...
bool XcpDaq_QueueDequeue(uint16_t *len, uint8_t *data) {
  ...
  *len = dto_len;
  ...
  XcpDaq_Queue.tail =
      (XcpDaq_Queue.tail + UINT8(1)) % UINT8(XCP_DAQ_QUEUE_SIZE + 1);
  return (bool)XCP_TRUE;
}
//#endif /* XCP_DAQ_ENABLE_DYNAMIC_LISTS */
#endif /* XCP_DAQ_ENABLE_QUEUING */
