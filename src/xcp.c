...
uint8_t *Xcp_GetDtoOutPtr(void) { return &(Xcp_DtoOut.data[XCP_TRANSPORT_LAYER_BUFFER_OFFSET]); }

void Xcp_SetDtoOutLen(uint16_t len) { Xcp_DtoOut.len = len; }
#endif /* XCP_ENABLE_DAQ_COMMANDS */

uint8_t *Xcp_GetCtoOutPtr(void) { return &(Xcp_CtoOut.data[XCP_TRANSPORT_LAYER_BUFFER_OFFSET]); }
...
#if XCP_ENABLE_SLAVE_BLOCKMODE == XCP_ON
XCP_STATIC bool Xcp_SlaveBlockTransferIsActive(void) { return Xcp_State.slaveBlockModeState.blockTransferActive; }
...
void Xcp_SetSendCallout(Xcp_SendCalloutType callout) { Xcp_SendCallout = callout; }
...
**
*/
Xcp_ConnectionStateType Xcp_GetConnectionState(void) { return Xcp_ConnectionState; }
...
XCP_STATIC void Xcp_Connect_Res(Xcp_PduType const *const pdu) {
    ...
    /*DBG_PRINT("MAX-DTO: %04X H: %02X L: %02X\n\r", XCP_MAX_DTO, HIBYTE(XCP_MAX_DTO), LOBYTE(XCP_MAX_DTO)); */
}
...
XCP_STATIC void Xcp_GetStatus_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(6), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), /* Current session status */
#if XCP_ENABLE_RESOURCE_PROTECTION == XCP_ON
...
XCP_STATIC void Xcp_GetId_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), UINT8(0), UINT8(0), XCP_LOBYTE(XCP_LOWORD(response_len)),
...
void Xcp_SendChecksumPositiveResponse(Xcp_ChecksumType checksum) {
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(XCP_CHECKSUM_METHOD), UINT8(0), UINT8(0),
...
void Xcp_SendChecksumOutOfRangeResponse(void) {
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_ERR), UINT8(ERR_OUT_OF_RANGE), UINT8(0), UINT8(0),
...
XCP_STATIC void Xcp_UserCmd_Res(Xcp_PduType const *const pdu) { XCP_ASSERT_PGM_IDLE(); }
#endif /* XCP_ENABLE_USER_CMD */
...
XCP_STATIC void Xcp_SetDaqListMode_Res(Xcp_PduType const *const pdu) {
    ...
    DBG_TRACE6("SET_DAQ_LIST_MODE [mode: 0x%02x daq: %03u event: %03u prescaler: %03u priority: %03u]\n\r", mode, daqListNumber,
               eventChannelNumber, prescaler, priority);
    XCP_ASSERT_DAQ_STOPPED();
    XCP_ASSERT_PGM_IDLE();
    XCP_ASSERT_UNLOCKED(XCP_RESOURCE_DAQ);
...
XCP_STATIC void Xcp_GetDaqProcessorInfo_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), properties, XCP_LOBYTE(listCount), XCP_HIBYTE(listCount),
...
#if XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), properties, listConf->numOdts, 0, /* Hardcoded for now. */
              0,                                                                            /* Hardcoded for now. */
...
XCP_STATIC void Xcp_GetDaqClock_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), UINT8(0), UINT8(0), XCP_LOBYTE(XCP_LOWORD(timestamp)),
...
XCP_STATIC void Xcp_GetDaqResolutionInfo_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(1), /* Granularity for size of ODT entry (DIRECTION = DAQ) */
...
XCP_STATIC void Xcp_GetDaqEventInfo_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(7), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(event->properties), /* DAQ_EVENT_PROPERTIES */
...
XCP_STATIC void Xcp_ProgramStart_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(7), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), /* Reserved */
...
XCP_STATIC void Xcp_GetPgmProcessorInfo_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(3), UINT8(XCP_PACKET_IDENTIFIER_RES), XCP_PGM_PROPERIES, /* PGM_PROPERTIES */
...
XCP_STATIC void Xcp_GetSectorInfo_Res(Xcp_PduType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(sectorNumber * 2), /* Clear Sequence Number */
...
XCP_STATIC void Xcp_ProgramFormat_Res(Xcp_PduType const *const pdu) {
    ...
    DBG_TRACE5("PROGRAM_FORMAT [compression: %d encryption: %d programming: %d access: %d]\n\r", compressionMethod,
               encryptionMethod, programmingMethod, accessMethod);
    XCP_ASSERT_PGM_ACTIVE();
...
INLINE uint8_t Xcp_GetByte(Xcp_PduType const *const pdu, uint8_t offs) { return (*(pdu->data + offs)); }
...
INLINE void Xcp_SetByte(Xcp_PduType const *const pdu, uint8_t offs, uint8_t value) { (*(pdu->data + offs)) = value; }
...
#if XCP_ENABLE_RESOURCE_PROTECTION == XCP_ON
XCP_STATIC bool Xcp_IsProtected(uint8_t resource) { return ((Xcp_State.resourceProtection & resource) == resource); }
#endif /* XCP_ENABLE_RESOURCE_PROTECTION */
...
XCP_STATIC void Xcp_ErrorResponse(uint8_t errorCode) {
    Xcp_Send8(UINT8(2), UINT8(XCP_PACKET_IDENTIFIER_ERR), UINT8(errorCode), UINT8(0), UINT8(0), UINT8(0), UINT8(0), UINT8(0),
              UINT8(0));
}
