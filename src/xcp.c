...
*/
XCP_STATIC uint8_t Xcp_SetResetBit8(uint8_t result, uint8_t value,
                                    uint8_t flag);

XCP_STATIC Xcp_MemoryMappingResultType Xcp_MapMemory(Xcp_MtaType const *src,
                                                     Xcp_MtaType *dst);
...
XCP_STATIC void Xcp_SendSpecialPacket(uint8_t packetType, uint8_t code,
...
XCP_STATIC void Xcp_WriteDaqEntry(uint8_t bitOffset, uint8_t elemSize,
                                  uint8_t adddrExt, uint32_t address);
...
#endif
#else
    Xcp_CommandNotImplemented_Res, Xcp_CommandNotImplemented_Res,
    Xcp_CommandNotImplemented_Res, Xcp_CommandNotImplemented_Res,
    Xcp_CommandNotImplemented_Res, Xcp_CommandNotImplemented_Res,
    Xcp_CommandNotImplemented_Res, Xcp_CommandNotImplemented_Res,
    Xcp_CommandNotImplemented_Res, Xcp_CommandNotImplemented_Res,
...
#endif
#if (XCP_ENABLE_DAQ_COMMANDS == XCP_ON) &&                                     \
    (XCP_ENABLE_WRITE_DAQ_MULTIPLE == XCP_ON)
...
void Xcp_SetMta(Xcp_MtaType mta) { Xcp_State.mta = mta; }
...
void Xcp_SendCto(void) {
  ...
  Xcp_CtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
      XCP_LOBYTE(Xcp_State.counter);
  Xcp_State.counter++;
#elif XCP_TRANSPORT_LAYER_COUNTER_SIZE == 2
  Xcp_CtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
      XCP_LOBYTE(Xcp_State.counter);
  Xcp_CtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE + 1] =
      XCP_HIBYTE(Xcp_State.counter);
  Xcp_State.counter++;
#endif /* XCP_TRANSPORT_LAYER_COUNTER_SIZE */
  ...
  XcpTl_Send(Xcp_CtoOut.data,
             Xcp_CtoOut.len + (uint16_t)XCP_TRANSPORT_LAYER_BUFFER_OFFSET);
}
...
void Xcp_SendDto(void) {
...
#if XCP_TRANSPORT_LAYER_COUNTER_SIZE == 1
  Xcp_DtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
      XCP_LOBYTE(Xcp_State.counter);
  Xcp_State.counter++;
#elif XCP_TRANSPORT_LAYER_COUNTER_SIZE == 2
  Xcp_DtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
      XCP_LOBYTE(Xcp_State.counter);
  Xcp_DtoOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE + 1] =
      XCP_HIBYTE(Xcp_State.counter);
  Xcp_State.counter++;
#endif /* XCP_TRANSPORT_LAYER_COUNTER_SIZE */
  ...
  XcpTl_Send(Xcp_DtoOut.data,
             Xcp_DtoOut.len + (uint16_t)XCP_TRANSPORT_LAYER_BUFFER_OFFSET);
}
...
void Xcp_SetDtoOutLen(uint16_t len) { Xcp_DtoOut.len = len; }
...
void Xcp_SetCtoOutLen(uint16_t len) { Xcp_CtoOut.len = len; }

void Xcp_Send8(uint8_t len, uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3,
...
XCP_STATIC void Xcp_Connect_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(resource),
            UINT8(commModeBasic), UINT8(XCP_MAX_CTO), XCP_LOBYTE(XCP_MAX_DTO),
            XCP_HIBYTE(XCP_MAX_DTO), UINT8(XCP_PROTOCOL_VERSION_MAJOR),
...
XCP_STATIC void Xcp_GetStatus_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(
      UINT8(6), UINT8(XCP_PACKET_IDENTIFIER_RES),
...
XCP_STATIC void Xcp_GetCommModeInfo_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), /* Reserved */
            commModeOptional, UINT8(0),                           /* Reserved */
            UINT8(XCP_MAX_BS), UINT8(XCP_MIN_ST), UINT8(XCP_QUEUE_SIZE),
...
XCP_STATIC void Xcp_GetId_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), UINT8(0),
            UINT8(0), XCP_LOBYTE(XCP_LOWORD(response_len)),
...
XCP_STATIC void Xcp_Unlock_Res(Xcp_PduType const *const pdu) {
  ...
  if (Xcp_HookFunction_Unlock(Xcp_State.seedRequested,
                              &key)) { /* User supplied callout. */
    Xcp_State.resourceProtection &=
        UINT8(~(Xcp_State.seedRequested)); /* OK, unlock. */
    Xcp_Send8(
        UINT8(2), UINT8(XCP_PACKET_IDENTIFIER_RES),
        Xcp_State.resourceProtection, /* Current resource protection status. */
...
XCP_STATIC void Xcp_ShortUpload_Res(Xcp_PduType const *const pdu) {
  ...
  XCP_ASSERT_PGM_IDLE();
  XCP_CHECK_MEMORY_ACCESS(Xcp_State.mta, len, XCP_MEM_ACCESS_READ,
                          (bool)XCP_FALSE);
  if (len > UINT8(XCP_MAX_CTO - 1)) {
    Xcp_ErrorResponse(UINT8(ERR_OUT_OF_RANGE));
    return;
...
void Xcp_SendChecksumPositiveResponse(Xcp_ChecksumType checksum) {
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES),
            UINT8(XCP_CHECKSUM_METHOD), UINT8(0), UINT8(0),
            XCP_LOBYTE(XCP_LOWORD(checksum)), XCP_HIBYTE(XCP_LOWORD(checksum)),
            XCP_LOBYTE(XCP_HIWORD(checksum)), XCP_HIBYTE(XCP_HIWORD(checksum)));
}

void Xcp_SendChecksumOutOfRangeResponse(void) {
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_ERR), UINT8(ERR_OUT_OF_RANGE),
            UINT8(0), UINT8(0),
...
XCP_STATIC void Xcp_BuildChecksum_Res(Xcp_PduType const *const pdu) {
  ...
  XCP_ASSERT_PGM_IDLE();
  XCP_CHECK_MEMORY_ACCESS(Xcp_State.mta, blockSize, XCP_MEM_ACCESS_READ,
                          (bool)XCP_FALSE);
  ...
  checksum =
      Xcp_CalculateChecksum(ptr, blockSize, (Xcp_ChecksumType)0, XCP_TRUE);
  Xcp_SendChecksumPositiveResponse(checksum);
#else
  if (blockSize <= UINT32(XCP_CHECKSUM_CHUNK_SIZE)) {
    checksum = Xcp_CalculateChecksum(ptr, blockSize, (Xcp_ChecksumType)0,
                                     (bool)XCP_TRUE);
    Xcp_SendChecksumPositiveResponse(checksum);
  } else {
...
XCP_STATIC void Xcp_Download_Res(Xcp_PduType const *const pdu) {
  ...
  if (len > LIMIT) {
    ...
    Xcp_State.masterBlockModeState.remaining =
        len - XCP_DOWNLOAD_PAYLOAD_LENGTH;
    Xcp_Download_Copy(UINT32(pdu->data + 2), UINT8(0),
                      UINT32(XCP_DOWNLOAD_PAYLOAD_LENGTH));
    return;
  }
#endif /* XCP_ENABLE_MASTER_BLOCKMODE */
  ...
  if (len > XCP_DOWNLOAD_PAYLOAD_LENGTH) {
    Xcp_ErrorResponse(
        ERR_OUT_OF_RANGE); /* Request exceeds max. payload size. */
    return;
  }
#endif
...
XCP_STATIC void Xcp_DownloadNext_Res(Xcp_PduType const *const pdu) {
  ...
  if (!Xcp_State.masterBlockModeState.blockTransferActive) {
    Xcp_ErrorResponse(
        ERR_SEQUENCE); /* Check: Is it really necessary to start a block-mode
...
XCP_STATIC void Xcp_DownloadMax_Res(Xcp_PduType const *const pdu) {
  ...
  XCP_ASSERT_PGM_IDLE();
  Xcp_Download_Copy(UINT32(pdu->data + 1), UINT8(0),
                    UINT32(XCP_DOWNLOAD_PAYLOAD_LENGTH + 1));
  Xcp_PositiveResponse();
}
...
XCP_STATIC void Xcp_ModifyBits_Res(Xcp_PduType const *const pdu) {
  ...
  DBG_TRACE4("MODIFY-BITS [shiftValue: 0x%02X andMask: 0x%04x ext: xorMask: "
             "0x%04x]\n\r",
...
XCP_STATIC void Xcp_SetDaqListMode_Res(Xcp_PduType const *const pdu) {
  ...
  const XcpDaq_ListIntegerType daqListNumber =
      (XcpDaq_ListIntegerType)Xcp_GetWord(pdu, UINT8(2));
  ...
  DBG_TRACE6("SET_DAQ_LIST_MODE [mode: 0x%02x daq: %03u event: %03u prescaler: "
             "%03u priority: %03u]\n\r",
...
*/
  ...
  entry->mode =
      Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_TIMESTAMP);
  entry->mode =
      Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_ALTERNATING);
  entry->mode =
      Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_DIRECTION);
...
XCP_STATIC void Xcp_StartStopDaqList_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), firstPid, UINT8(0),
...
XCP_STATIC void Xcp_GetDaqProcessorInfo_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), properties,
            XCP_LOBYTE(listCount), XCP_HIBYTE(listCount),
...
#if XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON
  Xcp_Send8(UINT8(8), UINT8(XCP_PACKET_IDENTIFIER_RES), properties,
            listConf->numOdts, 0, /* Hardcoded for now. */
            0,                    /* Hardcoded for now. */
...
XCP_STATIC void Xcp_AllocOdtEntry_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_SendResult(
      XcpDaq_AllocOdtEntry(daqListNumber, odtNumber, odtEntriesCount));
}
...
XCP_STATIC void Xcp_GetDaqClock_Res(Xcp_PduType const *const pdu) {
  ...
  Xcp_Send8(
      ...
      XCP_LOBYTE(XCP_LOWORD(timestamp)), XCP_HIBYTE(XCP_LOWORD(timestamp)),
...
XCP_STATIC void Xcp_ProgramFormat_Res(Xcp_PduType const *const pdu) {
  ...
  DBG_TRACE5("PROGRAM_FORMAT [compression: %d encryption: %d programming: %d "
             "access: %d]\n\r",
             compressionMethod, encryptionMethod, programmingMethod,
             accessMethod);
  XCP_ASSERT_PGM_ACTIVE();
...
INLINE uint16_t Xcp_GetWord(Xcp_PduType const *const pdu, uint8_t offs) {
  ...
  return ((*(pdu->data + offs)) & UINT8(0xff)) |
         (((*(pdu->data + UINT8(1) + offs)) & UINT8(0xff)) << UINT8(8));
#elif XCP_BYTE_ORDER == XCP_BYTE_ORDER_MOTOROLA
  return (((*(pdu->data + offs)) & UINT8(0xff)) << UINT8(8)) |
         ((*(pdu->data + UINT8(1) + offs)) & UINT8(0xff));
#endif
}
...
INLINE void Xcp_SetByte(Xcp_PduType const *const pdu, uint8_t offs,
                        uint8_t value) {
  (*(pdu->data + offs)) = value;
}

INLINE void Xcp_SetWord(Xcp_PduType const *const pdu, uint8_t offs,
                        uint16_t value) {
#if XCP_BYTE_ORDER == XCP_BYTE_ORDER_INTEL
...
INLINE void Xcp_SetDWord(Xcp_PduType const *const pdu, uint8_t offs,
                         uint32_t value) {
#if XCP_BYTE_ORDER == XCP_BYTE_ORDER_INTEL
...
XCP_STATIC bool Xcp_IsProtected(uint8_t resource) { return XCP_FALSE; }
...
bool Xcp_IsBusy(void) { return Xcp_State.busy; }
...
XCP_STATIC void Xcp_PositiveResponse(void) {
  Xcp_Send8(UINT8(1), UINT8(XCP_PACKET_IDENTIFIER_RES), UINT8(0), UINT8(0),
            UINT8(0), UINT8(0), UINT8(0), UINT8(0), UINT8(0));
}

XCP_STATIC void Xcp_ErrorResponse(uint8_t errorCode) {
  Xcp_Send8(UINT8(2), UINT8(XCP_PACKET_IDENTIFIER_ERR), UINT8(errorCode),
            UINT8(0), UINT8(0), UINT8(0), UINT8(0), UINT8(0), UINT8(0));
}
...
void Xcp_SendEventPacket(uint8_t eventCode, uint8_t const *const eventInfo,
                         uint8_t eventInfoLength) {
  Xcp_SendSpecialPacket(XCP_PACKET_IDENTIFIER_EV, eventCode, eventInfo,
                        eventInfoLength);
}
...
void Xcp_SendServiceRequestPacket(uint8_t serviceRequestCode,
                                  uint8_t const *const serviceRequest,
                                  uint8_t serviceRequestLength) {
  Xcp_SendSpecialPacket(XCP_PACKET_IDENTIFIER_SERV, serviceRequestCode,
                        serviceRequest, serviceRequestLength);
}
...
XCP_STATIC void Xcp_WriteDaqEntry(uint8_t bitOffset, uint8_t elemSize,
  ...
  DBG_TRACE5(
      "\tentry: [address: 0x%08x ext: 0x%02x size: %u bitOffset: %u]\n\r",
      address, adddrExt, elemSize, bitOffset);

  entry =
      XcpDaq_GetOdtEntry(Xcp_State.daqPointer.daqList, Xcp_State.daqPointer.odt,
                         Xcp_State.daqPointer.odtEntry);
