...
void XcpDaq_PrintDAQDetails(void) {
  ...
  printf("\n\rDAQ configuration\n\r");
  printf("-----------------\n\r");
  for (listIdx = (XcpDaq_ListIntegerType)0; listIdx < XcpDaq_GetListCount();
       ++listIdx) {
    ...
    printf("DAQ-List #%d [%s] firstPid: %d mode: ", listIdx,
           (listIdx < XCP_MIN_DAQ) ? "predefined" : "dynamic", firstPid);
#elif XCP_DAQ_CONFIG_TYPE == XCP_DAQ_CONFIG_TYPE_STATIC
    printf("DAQ-List #%d [%s] firstPid: %d mode: ", listIdx,
           (listIdx < XCP_MIN_DAQ) ? "predefined" : "static", firstPid);
    ...
    for (odtIdx = (XcpDaq_ODTIntegerType)0; odtIdx < listConf->numOdts;
      ...
      printf("    ODT #%d\n\r", odtIdx);
      for (odtEntriyIdx = (XcpDaq_ODTEntryIntegerType)0;
           odtEntriyIdx < odt->numOdtEntries; ++odtEntriyIdx) {
