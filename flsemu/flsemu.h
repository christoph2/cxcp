...
**  Global Defines.
*/
#define FLSEMU_ERASED_VALUE                                                    \
  (0xff) /**< Value of an erased Flash/EEPROM cell.                            \
          */
#define FLSEMU_FILLER_VALUE                                                    \
  (0xcc) /**< Value of an filler value (align to allocation granularity). */
...
#define FLSEMU_MB(v)                                                           \
  (((uint32_t)((v))) * (0x400UL * 0x400UL)) /**< Megabytes to bytes. */

/*
**  Local Function-like Macros.
*/
#define FLSEMU_ASSERT_INITIALIZED()                                            \
  assert(*FlsEmu_GetModuleState() == FLSEMU_INIT)
#define FLSEMU_VALIDATE_SEGMENT_IDX(idx)                                       \
  ((idx) < FlsEmu_GetConfig()->numSegments)
