...
**  Local Variables.
*/
static FlsEmu_ModuleStateType FlsEmu_ModuleState =
    FLSEMU_UNINIT; /**< Module-state variable. */
static FlsEmu_SystemMemoryType
    FlsEmu_SystemMemory; /**< System memory configuration. */
static FlsEmu_ConfigType const *FlsEmu_Config =
    XCP_NULL; /**< Segment configuration. */
...
void FlsEmu_OpenCreate(uint8_t segmentIdx) {
  ...
  segment->persistentArray =
      (FlsEmu_PersistentArrayType *)malloc(sizeof(FlsEmu_PersistentArrayType));
  ...
  if (result == OPEN_ERROR) {
    ...
    for (pageIdx = 0U; pageIdx < numPages; ++pageIdx) {
      // FIXME!!!
      offset = ((int)segment->persistentArray->mappingAddress) +
               (pageIdx * pageSize);
...
void FlsEmu_EraseSector(uint8_t segmentIdx, uint32_t address) {
  ...
  XcpUtl_MemSet(ptr + (address & ~mask), FLSEMU_ERASED_VALUE,
                segment->sectorSize);
}
...
void FlsEmu_ErasePage(uint8_t segmentIdx, uint8_t page) {
  ...
  segment = FlsEmu_GetConfig()->segments[segmentIdx];
  XcpUtl_MemSet(ptr + (segment->pageSize * page), FLSEMU_ERASED_VALUE,
                segment->pageSize);
...
Xcp_MemoryMappingResultType FlsEmu_MemoryMapper(Xcp_MtaType *dst,
                                                Xcp_MtaType const *src) {
  uint8_t idx = 0;
  uint8_t *ptr = XCP_NULL;
  FlsEmu_SegmentType *segment = XCP_NULL;
  ...
  for (idx = 0; idx < FlsEmu_GetConfig()->numSegments; ++idx) {
    ...
    if ((src->address >= segment->baseAddress) &&
        (src->address < (segment->baseAddress + segment->pageSize))) {
...
bool FlsEmu_Initialized(void) {
  FlsEmu_ModuleStateType *state = FlsEmu_GetModuleState();
