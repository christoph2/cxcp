...
static void
FlsEmu_ClosePersitentArray(FlsEmu_PersistentArrayType const *persistentArray);
static bool FlsEmu_MapAddress(FlsEmu_SegmentType *config, uint32_t offset,
                              uint32_t length);
static void FlsEmu_MemoryInfo(void const *address);
static MEM_HANDLE OpenCreateFile(char const *fileName, bool create);
static bool CreateFileView(MEM_HANDLE handle, DWORD length,
...
static bool FlsEmu_Flush(uint8_t segmentIdx) {
  ...
  if (VirtualQuery(segment->persistentArray->mappingAddress, &info,
                   sizeof(MEMORY_BASIC_INFORMATION)) == 0) {
    XcpHw_ErrorMsg("FlsEmu_Flush::VirtualQuery()", GetLastError());
    return FALSE;
  }

  if (!FlushViewOfFile(segment->persistentArray->mappingAddress,
                       info.RegionSize)) {
...
static bool FlsEmu_MapAddress(FlsEmu_SegmentType *config, uint32_t offset,
  ...
  config->persistentArray->mappingAddress =
      (void *)MapViewOfFile(config->persistentArray->mappingHandle,
                            FILE_MAP_ALL_ACCESS, 0, offset, length);
...
FlsEmu_OpenCreatePersitentArray(char const *fileName, uint32_t size,
...
static MEM_HANDLE OpenCreateFile(char const *fileName, bool create) {
  ...
  handle = CreateFile(fileName, GENERIC_READ | GENERIC_WRITE,
                      ...
                      (LPSECURITY_ATTRIBUTES)NULL, dispoition,
...
static bool CreateFileView(MEM_HANDLE handle, DWORD length,
                           FlsEmu_HwFileViewType *fileView) {
  fileView->mappingHandle =
      CreateFileMapping(handle, (LPSECURITY_ATTRIBUTES)NULL, PAGE_READWRITE,
                        (DWORD)0, (DWORD)length, NULL);
  if (fileView->mappingHandle == NULL) {
    return FALSE;
  }

  /* TODO: Refactor to function; s. FlsEmu_MapView() */
  fileView->mappingAddress = (void *)MapViewOfFile(
      fileView->mappingHandle, FILE_MAP_ALL_ACCESS, 0, 0, length);
