...
class FunctionGenerator {
   ...
   public:
    ...
    float generateSample(uint32_t sample_index) {
        ...
        float t_shifted   = t + phase_shift / omega;
        ...
        switch (m_type) {
            ...
            case TRIANGLE:
                ...
                case TRIANGLE: {
                    float t_shifted = t + phase_shift / omega;
                    float value = 2.0F * ::abs(2.0F * (t_shifted * m_params.frequency - ::floor(t_shifted * m_params.frequency +
    0.5F))) - 1.0F; return m_params.amplitude * value;
    }

                */

                return m_params.amplitude *
                       (2.0F * ::abs(2.0F * (t_shifted * m_params.frequency - ::floor(t_shifted * m_params.frequency + 0.5F))) -
                        1.0F);

                // return m_params.amplitude * (2.0F * ::abs(2.0F * (t * m_params.frequency - ::floor(t * m_params.frequency +
                // 0.5F))) - 1.0F);
            case SQUARE:
