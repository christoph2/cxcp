...
void Xcp_SendChecksumPositiveResponse(Xcp_ChecksumType checksum) {
  Xcp_Send8(UINT8(8), UINT8(0xff), UINT8(XCP_CHECKSUM_METHOD), UINT8(0),
            UINT8(0), XCP_LOBYTE(XCP_LOWORD(checksum)),
            XCP_HIBYTE(XCP_LOWORD(checksum)), XCP_LOBYTE(XCP_HIWORD(checksum)),
            XCP_HIBYTE(XCP_HIWORD(checksum)));
}

void Xcp_SendChecksumOutOfRangeResponse(void) {
  Xcp_Send8(UINT8(8), UINT8(0xfe), UINT8(ERR_OUT_OF_RANGE), UINT8(0), UINT8(0),
...
XCP_STATIC void Xcp_ModifyBits_Res(Xcp_PDUType const *const pdu) {
  ...
  DBG_TRACE4(
      "MODIFY-BITS [shiftValue: 0x%02X andMask: 0x%04x ext: xorMask: 0x%04x]\n",
...
XCP_STATIC void Xcp_WriteDaq_Res(Xcp_PDUType const *const pdu) {
  ...
  /* Advance ODT entry pointer within  one  and  the same ODT. After writing to
...
XCP_STATIC void Xcp_ProgramFormat_Res(Xcp_PDUType const *const pdu) {
  ...
  DBG_TRACE5(
      "PROGRAM_FORMAT [compression: %d encryption: %d programming: %d access: "
      "%d]\n",
