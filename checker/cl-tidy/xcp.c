...
void Xcp_SendPdu(void) {
    ...
    Xcp_PduOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
        XCP_LOBYTE(Xcp_State.counter);
    Xcp_State.counter++;
#elif XCP_TRANSPORT_LAYER_COUNTER_SIZE == 2
    Xcp_PduOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE] =
        XCP_LOBYTE(Xcp_State.counter);
    Xcp_PduOut.data[XCP_TRANSPORT_LAYER_LENGTH_SIZE + 1] =
    ...
    XcpTl_Send(Xcp_PduOut.data,
               Xcp_PduOut.len + (uint16_t)XCP_TRANSPORT_LAYER_BUFFER_OFFSET);
}
...
void Xcp_SetPduOutLen(uint16_t len) { Xcp_PduOut.len = len; }
...
XCP_STATIC void Xcp_Connect_Res(Xcp_PDUType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(0xff), UINT8(resource), UINT8(commModeBasic),
              UINT8(XCP_MAX_CTO), XCP_LOBYTE(XCP_MAX_DTO),
...
XCP_STATIC void Xcp_GetCommModeInfo_Res(Xcp_PDUType const *const pdu) {
    ...
    Xcp_Send8(UINT8(8), UINT8(0xff), UINT8(0), /* Reserved */
...
XCP_STATIC void Xcp_Unlock_Res(Xcp_PDUType const *const pdu) {
    ...
    if (Xcp_HookFunction_Unlock(Xcp_State.seedRequested,
        ...
        Xcp_Send8(
            UINT8(2), UINT8(0xff),
            Xcp_State
                .resourceProtection, /* Current resource protection status. */
...
void Xcp_SendChecksumPositiveResponse(Xcp_ChecksumType checksum) {
    Xcp_Send8(
        UINT8(8), UINT8(0xff), UINT8(XCP_CHECKSUM_METHOD), UINT8(0), UINT8(0),
...
void Xcp_SendChecksumOutOfRangeResponse(void) {
    Xcp_Send8(UINT8(8), UINT8(0xfe), UINT8(ERR_OUT_OF_RANGE), UINT8(0),
              UINT8(0), XCP_LOBYTE(XCP_LOWORD(XCP_CHECKSUM_MAXIMUM_BLOCK_SIZE)),
...
XCP_STATIC void Xcp_Download_Res(Xcp_PDUType const *const pdu) {
    ...
    if (len > (XCP_MAX_BS * XCP_DOWNLOAD_PAYLOAD_LENGTH)) {
        Xcp_ErrorResponse(
            ERR_OUT_OF_RANGE); /* Request exceeds max. block size. */
        return;
...
XCP_STATIC void Xcp_DownloadNext_Res(Xcp_PDUType const *const pdu) {
    ...
    if (!Xcp_State.masterBlockModeState.blockTransferActive) {
        Xcp_ErrorResponse(
            ERR_SEQUENCE); /* Check: Is it really necessary to start a
...
XCP_STATIC void Xcp_WriteDaq_Res(Xcp_PDUType const *const pdu) {
    ...
    entry = XcpDaq_GetOdtEntry(Xcp_State.daqPointer.daqList,
                               Xcp_State.daqPointer.odt,
                               Xcp_State.daqPointer.odtEntry);
...
*/
    ...
    entry->mode =
        Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_PID_OFF);
    entry->mode =
        Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_SELECTED);
    entry->mode =
        Xcp_SetResetBit8(entry->mode, mode, XCP_DAQ_LIST_MODE_STARTED);
...
2  WORD  DAQ_LIST_NUMBER [0,1,..MAX_DAQ-1]
    ...
    Xcp_Send8(UINT8(8), UINT8(0xff), firstPid, UINT8(0), UINT8(0), UINT8(0),
...
XCP_STATIC void Xcp_GetDaqProcessorInfo_Res(Xcp_PDUType const *const pdu) {
    ...
    7  BYTE  DAQ_KEY_BYTE
#endif /* 0 */
    Xcp_Send8(UINT8(8), UINT8(0xff), properties, XCP_LOBYTE(listCount),
...
#if XCP_DAQ_ENABLE_PREDEFINED_LISTS == XCP_ON
    Xcp_Send8(UINT8(8), UINT8(0xff), properties, listConf->numOdts,
              0, /* Hardcoded for now. */
              0, /* Hardcoded for now. */
              UINT8(0), UINT8(0), UINT8(0));
#else
...
XCP_STATIC void Xcp_GetDaqClock_Res(Xcp_PDUType const *const pdu) {
    ...
    Xcp_Send8(
        UINT8(8), UINT8(0xff), UINT8(0), UINT8(0), UINT8(0),
...
XCP_STATIC void Xcp_GetPgmProcessorInfo_Res(Xcp_PDUType const *const pdu) {
    ...
    Xcp_Send8(UINT8(3), UINT8(0xff), XCP_PGM_PROPERIES, /* PGM_PROPERTIES */
...
XCP_STATIC void Xcp_PositiveResponse(void) {
    Xcp_Send8(UINT8(1), UINT8(0xff), UINT8(0), UINT8(0), UINT8(0), UINT8(0),
              UINT8(0), UINT8(0), UINT8(0));
}

XCP_STATIC void Xcp_ErrorResponse(uint8_t errorCode) {
    Xcp_Send8(UINT8(2), UINT8(0xfe), UINT8(errorCode), UINT8(0), UINT8(0),
